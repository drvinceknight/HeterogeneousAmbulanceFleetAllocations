...
def test_run_experiment_full_mutation():
    """
    Goes up to root dir and runs command. Captures stdout and confirms exit code
    is 0.
    ...
    args = [
        "python",
        "src/experiment-full-mutation.py",
        "11",
        "1",
        "1",
        "2",
    ...
    # Now checks the results are written and in the correct format.
    best_primary = pd.read_csv("../results/allocation_primary_44444.csv")
    best_secondary = pd.read_csv("../results/allocation_secondary_44444.csv")
    population_objectives = pd.read_csv("../results/population_objectives_44444.csv")
    expected_headers_allocations = [
        "scenario_id",
        "demand_scenario",
        "resource_level",
    ...
    assert len(best_primary.columns) == 77
    assert len(best_secondary.columns) == 77
    for c in range(77):
        assert best_primary.columns[c] == expected_headers_allocations[c]
        assert best_secondary.columns[c] == expected_headers_allocations[c]
    assert len(population_objectives.columns) == 13
    for c in range(13):
        assert population_objectives.columns[c] == expected_headers_population[c]
    assert len(best_primary) == 1
    assert len(best_secondary) == 1
    ...
    assert total_primary + (total_secondary / 3) == 11
