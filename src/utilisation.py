...
def constant_utilisation_primary(allocation_primary, utilisation_rate_primary, **kwargs):
    """
...
def constant_utilisation_secondary(allocation_secondary, utilisation_rate_secondary, **kwargs):
    """
...
def proportional_utilisations_primary(allocation_primary, demand_rates, service_rate_primary, **kwargs):
    """
    Returns a vector the proportions the primary demand equally across the allocation
    """
    total_demand = demand_rates.sum()
    return np.array(
        [0 if z == 0 else total_demand / (service_rate_primary * z) for z in allocation_primary]
    )

def proportional_utilisations_secondary(allocation_secondary, demand_rates, service_rate_secondary, **kwargs):
    """
    Returns a vector the proportions the secondary demand equally across the allocation
    """
    total_demand = demand_rates[:-1].sum()
    return np.array(
        [0 if z == 0 else total_demand / (service_rate_secondary * z) for z in allocation_secondary]
    )