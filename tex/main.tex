...
respectively. Note that here we assume that average job times are not dependant
on the vehicle location $a$.
...
We note that the assumption that the average job times are not dependant on the
vehicle location $a$ may be unrealistic, given that the travel between locations
are a key part of an vehicle's job time ($B_{pa}$, $D_{ya}$, $F_{pa}$). However,
as discussed in Section~\ref{sec:problem_description}, there are a number of
other components to an ambulance job including time on site ($G_k$), hospital
handover time ($J_{yk}$), and vehicle refill time ($\Theta$). We justify the
assumption of job times not being location dependant by considering the
proportion of an ambulance job's time that is location dependant, $\rho$,
