...
%A self-contained abstract of about 100-200 words outlining the aims, scope and conclusions of the paper must be supplied..
 ...
 although the ambulance allocation problem has been widely studied,
 ...
 paper we propose a modelling approach to consider where to best allocate
 ...
 metaheuristic algorithm for finding better fleet allocations which is used in
 conjunction with a discrete-event simulation model of ambulance services with
 heterogeneous vehicles. A major contribution of this metaheuristic is the numerical
 ...
 and allocations, and their impact on patient outcomes.
...
forecast emergency demand and make critical decisions on the best types,
capacities and geographical allocations of emergency vehicles within a
...
on the development of a simulation model and allocation metaheuristic
...
McCormack and Coates~(2015)~\cite{MCormack2015} focus on the optimisation of
...
Toro-D\'iaz et al.~(2013, 2015)~\cite{torodiaz13, torodiaz15} build
...
response times, expected coverage \cite{daskin83}, and the Gini index on
...
justified assumption for the case of Jakarta \cite{BriceSyaribahNoor2022Esui}.
...
\begin{figure}
    \centering
    \includestandalone[width=0.5\textwidth]{img/diagram}
...
\cite{Knight2012918}, and gives the probability of survival if seen within a
...
\centering
  \includegraphics[width=0.4\textwidth]{img/Survival_Function.pdf}
    \caption{Survival function $s(t)$, estimated by \cite{Valenzuela20001206}
             compared with $s_L(t)$ with a hard cutoff of $L=8$, represented as
             a step function.}
...
the preference of sending a vehicle from one station to another, that is the
dispatch rule. Here we use the closest or fastest vehicle, so
...
than a secondary vehicle at $a_2$, defined in Equation~\ref{eqn:R}. Note that
here these parameters are defined using travel times, but can be generalised to
account for any type of preference, including for example proximity, importance,
or efficiency.
...
\cite{2020SciPy-NMeth}. It should be noted that in this section we assume that
...
\For{$i \leftarrow 0$ \KwTo $H$}{
  ...
  $m \leftarrow \lceil m_0 - \max\left(m_0, i \times c\right)\rceil$\;
...
Separating out primary and secondary vehicle logic simplifies the model logic,
allows for modular simulations that can be run in isolation, and so easier to
adapt and to maintain.
...
servers. This is similar to the approach of \cite{kergosien15}, whereas
...
% \begin{tabular}{cccc}
% \toprule
% Demand Scenario & Mean Response Time &  MESLMHPHF objective & Percent Abandoned \\
% \midrule
% \textbf{D13} & 116 (116, 0)  & 60, (36, 72) & 76, (76, 0) \\
% \textbf{D19} & 120 (70, 150) & 60, (40, 60) & 94, (94, 0) \\
% \textbf{D34} & 92 (55, 111)  & 60, (43, 51) & $>$ 124 \\
% \textbf{D45} & 76 (52, 72)   & 60, (52, 24) & $>$ 124 \\
% \midrule
% Current Level & 17.69 minutes & 98.34\% & 0.00\% \\
% \bottomrule
% \end{tabular}%
% }
...
\cite{Knight2012918} to include more than one vehicle type. A particular novelty
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{img/hyperparameter_exploration}
