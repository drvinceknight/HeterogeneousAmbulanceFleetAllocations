...
\title[On the Optimisation of Heterogeneous Ambulance Fleet Allocations]{On the Optimisation of Heterogeneous Ambulance Fleet Allocations}
...
\begin{figure}
    \centering
    \includestandalone[width=0.475\textwidth]{img/diagram}
...
\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{img/Survival_Function.pdf}
...
from a pickup location. In both Equation~\ref{eqn:beta} and~\ref{eqn:R}, $\leq$ are used rather than $<$; in Equation~\ref{eqn:beta} will be equivalent, as we assume no two ambulance locations are equidistant; while in Equation~\ref{eqn:R}, in the case of a primary and secondary vehicle having equal travel times, then the primary vehicle will have priority.
Note that here these parameters are defined using
...
\For{$i \leftarrow 0$ \algorithmicto $H$}
  ...
  \For{$j \leftarrow 0$ \algorithmicto $N - \kappa$}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.4\textwidth,trim={0 6cm 0 5cm}, clip]{img/jakarta_region_names.pdf}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.8\textwidth]{img/map_current}
...
\begin{itemize}
  ...
  time of 8 minutes. In our data set there were 168 identified calls that met
        this criterion. As in~\cite{Knight2012918},
...
\begin{figure}
\begin{center}
\includegraphics[width=0.85\textwidth]{img/yearly_demand.pdf}
\end{center}
...
year, with a month warm up time, over five replications. 
The variability over the replications is very small, and we have included 90\% conficence intervals around the mean response times to demonstrate this.
We observe that the
...
RRV Utilisation & 22.47\% & - & - \\[1.5mm]
\multirow{2}{*}{Mean Response Time (mins)} & 17.69 & 22.29 & 24.05 \\
 & ($\pm$ 0.07) & ($\pm$ 0.06) & ($\pm$ 0.03) \\[1.5mm]
StDev Response Time (mins) & 6.98 & 8.02 & 7.94 \\
MESLMHPHF Objective & 98.34\% & 87.95\% & 85.64\% \\
MESLMHPHF A1 Objective & 26.05\% & 13.64\% & 12.23\% \\
...
allocations, for each demand scenario, along with 90\% confidence intervals over the trials. It can be seen that, as expected, as
...
The value of the MESLMHPHF objective function and
...
As desired, the optimisation heuristic has indeed improved the
allocation as measured by the MESLMHPHF objective function. In particular A1 patients 
are benefiting from the improved
allocations as measured by the MESLMHPHF objective function,
though not by the simulation. Why the simulation might not capture this is
hypothesised in Section~\ref{sec:compare_exp_and_sim}. This measured improvement
is due to these patients being more sensitive to small fluctuations in
...
more numerical evidence would be helpful here. This improved robustness 
...
\begin{table}
\resizebox{\textwidth}{!}{%
\input{tbl/current_vs_optimised.tex}%
}
...
Therefore we present two separate models to measure overall
survival, both of which have drawbacks in terms of capturing the stochasticity
of the system. We present both as a holistic view of the system, and suggest the
true outcomes lie somewhere in between the two measures. As the heuristic
improves the outcomes in one measure and does no harm in the other, we
anticipate that this procedure will improve patient outcomes.
...
over the replications were too small to display on the plots. It can
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[width=0.4\textwidth]{img/plots/multiple_RRVUtilisation}
\end{center}
...
Combinations of models allow for robust conclusions to be drawn
about the system, by considering the specific attributes of each modelling
approach. Here, the MESLMHPHF is quick to run, but can fail to capture
stochasticity, while the simulation is parametrised using expert opinions that
can effect its robustness. Taking both into account offers a better
approximation and more insight.
...
The data and code underlying this article are openly available in two Github repositories. The simulation can be found at \url{https://github.com/MarkTuson/ambulance_simulation}, and the MESLMHPHF and optimisation can be found at \url{https://github.com/drvinceknight/HeterogeneousAmbulanceFleetAllocations}.
...
\begin{figure}[b]
\centering
  \includegraphics[width=0.45\textwidth]{img/time_on_site_fit.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{img/hyperparameter_exploration}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.65\textwidth]{img/map_optimised_13}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.65\textwidth]{img/map_optimised_19}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.65\textwidth]{img/map_optimised_34}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.65\textwidth]{img/map_optimised_45}
